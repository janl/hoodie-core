...
exports["setup"] = function hoodie_setup()
{
  ...
  // create /etc/resolver/#{name}.hoodie.dev
    ...
    exec(cmd, function(error, stdout, stderr) {
      ...
      // clone matching hoodie-app-skeleton to share/
      var hoodir = process.env["HOME"] + "/.hoodie";
      mkdir("-p", hoodir);
      cd(hoodir);
      var cmd;
      if(fs.existsSync("./hoodie-app-skeleton")) {
        cd("hoodie-app-skeleton");
        cmd = "git pull --rebase"; // TBD link to hoodie core version
      } else {
        cmd = "git clone https://github.com/hoodiehq/hoodie-app-skeleton.git";
      }
      exec(cmd, function(error, stdout, stderr) {
        if(error) throw error;
        chown("-r", process.env["SUDO_USER"], hoodir);
  ...
  // chown $SUDO_USER ~/.hoodie.json
  chown(process.env["SUDO_USER"], process.env["HOME"] + "/.hoodie.json");
};
...
{
  // create app
  var skeleton_path = process.env["HOME"] + "/.hoodie/hoodie-app-skeleton";
  var cmd = util.format("git clone %s %s", skeleton_path, name);
  exec(cmd, function(error, stdout, stderr) {
    if(error) {
      throw(error);
    }
...
{
  return daemon.setup({
    main: __dirname + "/../bin/hoodie-service.js",
